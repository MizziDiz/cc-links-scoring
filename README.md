...
### Отсев до загрузки: выход паттерна и новые домены

На продакшн-прогоне 25–30 июля 2026 года 46% загруженных страниц вернулись как
`unmatched`, и почти весь этот объём пришёлся на несколько групп «паттерн
discovery × ярус» (один `guestbook` яруса 0 — 541 тыс. загрузок при выходе
4,5%). `yield_report.py` читает базу только для чтения, показывает выход по
группам и считает, что дал бы порог на той же истории:

```
python yield_report.py --db prospects.db --since 2026-07-25 \
    --thresholds 0.1 0.2 0.3 --out pattern-yield.json
```

`--pattern-min-yield 0.2` заставляет `prospect_pipeline.py` / `multi_crawl.py`
не загружать URL из групп, чей исторический выход ниже 20%. Группы с числом
решений меньше `--pattern-min-decisions` (по умолчанию 20) загружаются всегда,
а из отсечённых групп детерминированная доля `--pattern-explore-share` (5%)
всё равно проходит, чтобы группа могла «реабилитироваться». Без
`--pattern-yield-file` профиль строится из `--db` при старте, так что он
обновляется с каждым краулом. **После смены таксономии `--pattern-yield-since`
надо перенести на момент её развёртывания**: выход считается по идентификатору
паттерна, и правило, которое новая таксономия починила (гостевые книги: 4,5%
при v2, 97% при v3), иначе будет отсекаться по старой истории. Пока решений
с новой таксономией меньше `--pattern-min-decisions`, группа загружается
целиком. Порог ничего не меняет в классификаторе,
`--min-score` и `--max-per-domain`: он только решает, за что платить запросом.

Если целевая метрика — уникальные площадки, а не число URL, добавьте
`--new-domains-only` (не загружать URL доменов, у которых уже есть кандидат)
